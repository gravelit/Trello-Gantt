...
def adjust_end_date(end):
    SATURDAY = 5
    SUNDAY = 6

    # If the end date is on a Saturday or Sunday, move it forward to the monday
    if end.weekday() == SATURDAY:
        end = end + timedelta(days=2)
    if end.weekday() == SUNDAY:
...
def gantt_colorscale(
    ...
    if isinstance(chart[0][index_col], Number):
        ...
        for index in range(len(tasks)):
            ...
            scatter_data_dict[color_id]["legendgroup"] = color_id
            scatter_data_dict[color_id]["text"] = tasks[index]['description']
    ...
    if isinstance(chart[0][index_col], str):
        ...
        for index in range(len(tasks)):
            ...
            scatter_data_dict[color_id]["legendgroup"] = color_id
            scatter_data_dict[color_id]["name"] = str(chart[index][index_col])
            scatter_data_dict[color_id]["taskname"] = tn
            if percent:
                scatter_data_dict[color_id]["percent"] = chart[index]['Percent']
    ...
    if percent:
        ...
        for item in data:
            if 'taskname' in item:
                ...
                start = datetime.strptime(entry['x'][0], '%Y-%m-%d').timestamp()
                end   = datetime.strptime(entry['x'][1], '%Y-%m-%d').timestamp()
                delta = end - start
                actual = int(start + (delta * item['percent']))

                entry['x'][1] = str(datetime.fromtimestamp(actual))
                entry['x'][2] = str(datetime.fromtimestamp(actual))
